...
def run_command(command, error_message):
    """
    Runs a shell command. It now checks both the exit code and stderr for common error keywords
    to more reliably detect failures.
    """
    try:
        # Run the command without `check=True` to inspect the result manually.
        process = subprocess.run(command, shell=True, capture_output=True, text=True)
        
        # --- MODIFIED SECTION ---
        # A command is considered failed if its exit code is not 0, OR if its
        # error output contains specific failure keywords. This catches errors
        # like "port is already allocated" that might not cause a non-zero exit code.
        stderr_lower = process.stderr.lower()
        has_error = process.returncode != 0 or any(
            keyword in stderr_lower for keyword in ["error", "failed", "port is already allocated"]
        )
        
        if has_error:
            print(f"❌ Error: {error_message}")
            print("\n--- Command Output (Error) ---")
            # Show stderr if it has content, otherwise show stdout.
